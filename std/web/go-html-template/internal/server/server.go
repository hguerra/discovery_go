...
func NewServer() {
	...
	mux.HandleFunc("GET /search", func(w http.ResponseWriter, r *http.Request) {
		q := r.URL.Query().Get("q")
		data := M{"result": fmt.Sprintf("lorem %s: %s", q, time.Now().Format("2006-01-02T15:04:05Z"))}
		err := tpl.ExecuteTemplate(w, "_search", data)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
		}
	})
