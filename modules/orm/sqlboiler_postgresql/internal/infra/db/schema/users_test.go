...
func testUserToManyAuthorBlogPosts(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.AuthorBlogPosts().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadAuthorBlogPosts(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AuthorBlogPosts); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.AuthorBlogPosts = nil
	if err = a.L.LoadAuthorBlogPosts(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AuthorBlogPosts); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpAuthorBlogPosts(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddAuthorBlogPosts(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.AuthorBlogPosts[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.AuthorBlogPosts[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.AuthorBlogPosts().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpAuthorBlogPosts(t *testing.T) {
	var err error
	...
	err = a.SetAuthorBlogPosts(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetAuthorBlogPosts(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.AuthorBlogPosts[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.AuthorBlogPosts[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpAuthorBlogPosts(t *testing.T) {
	var err error
	...
	err = a.AddAuthorBlogPosts(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveAuthorBlogPosts(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.AuthorBlogPosts) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.AuthorBlogPosts[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.AuthorBlogPosts[0] != &e {
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `Name`: `character varying`, `LastName`: `character varying`, `Age`: `integer`}
	_           = bytes.MinRead
)
